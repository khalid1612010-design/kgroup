document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderServiceCategories() {
    ...
    refs.servicesCategoriesGrid.innerHTML = SERVICE_CATEGORIES.map(category => {
      ...
      return `
        <article class="service-category-card" data-service-category="${category.id}" role="button" tabindex="0">
          <div class="service-category-media">
            ${cover ? `<img src="${escapeHtml(cover)}" alt="${label}" loading="lazy" decoding="async">` : ''}
          </div>
  ...
  function renderServicesGallery() {
    ...
    if (!filtered.length) {
      refs.servicesGalleryGrid.innerHTML = '';
      refs.servicesGalleryEmpty.classList.remove('hidden');
      return;
    }

    refs.servicesGalleryEmpty.classList.add('hidden');
    refs.servicesGalleryGrid.innerHTML = filtered.map(item => `
      <article class="portfolio-card services-card">
        <div class="portfolio-media">
          <img src="${escapeHtml(item.image)}" alt="${escapeHtml(item.name)}" loading="lazy" decoding="async">
        </div>
        <div class="portfolio-body" style="padding: 1rem; text-align: center;">
          <h3 style="font-size: 1.15rem; font-weight: 700; margin: 0; color: var(--text);">${escapeHtml(item.name)}</h3>
        </div>
      </article>
    `).join('');
  }
