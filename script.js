document.addEventListener('DOMContentLoaded', () => {
  ...
  const refs = {
    ...
    servicesCategoryPills: document.getElementById('services-category-pills'),
  ...
  function renderServicesCategoryPills() {
    if (!refs.servicesCategoryPills) return;
    refs.servicesCategoryPills.innerHTML = SERVICE_CATEGORIES.map(category => `
      <button class="category-pill ${state.selectedServiceCategory === category.id ? 'active' : ''}" data-service-category="${category.id}">
        ${escapeHtml(currentText(category.nameAr, category.nameEn))}
      </button>
    `).join('');

    if (!refs.servicesCategoryPills.dataset.bound) {
      refs.servicesCategoryPills.dataset.bound = 'true';
      refs.servicesCategoryPills.addEventListener('click', event => {
        const button = event.target.closest('[data-service-category]');
        if (!button) return;
        state.selectedServiceCategory = button.dataset.serviceCategory;
        renderServicesCategoryPills();
        renderServices();
      });
    }
  ...
  function renderServices() {
    if (!refs.servicesGrid || !refs.servicesEmpty) return;

    const filtered = state.services.filter(item => item.category === state.selectedServiceCategory);

    if (!filtered.length) {
      refs.servicesGrid.innerHTML = '';
      refs.servicesEmpty.classList.remove('hidden');
      return;
    }

    refs.servicesEmpty.classList.add('hidden');
    refs.servicesGrid.innerHTML = filtered.map(item => `
  ...
  async function loadPublicCollections() {
    ...
    try {
      const [categories, products, portfolio, clientLogos, services] = await Promise.all([
  ...
  async function saveServiceFromForm() {
    ...
    state.selectedServiceCategory = payload.category;
